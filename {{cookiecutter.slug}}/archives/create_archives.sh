...
for d in */; do (cd $d && if [[ -f plan.py ]]; then stepup boot; fi); done
...
stepup make-inventory -i main-inventory.def -o main-inventory.txt
stepup zip-inventory main-inventory.txt archives/main-latest-with-outputs.zip

# Make a final inventory file of the archived data
cd archives
stepup make-inventory -o main-archive-inventory.txt README.md main.bundle main-latest-with-outputs.zip
